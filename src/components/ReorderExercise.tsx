...
const ReorderExercise = ({ number, words, acceptedAnswers, onScore }: Props) => {
  const [value, setValue] = useState("");
  const [validated, setValidated] = useState(false);

  const normalize = (s: string) => s.trim().toLowerCase().replace(/\s+/g, " ").replace(/\s*\?\s*$/, " ?");

  const handleValidate = () => {
    if (validated || !value.trim()) return;
    setValidated(true);
    const isCorrect = acceptedAnswers.some(a => normalize(a) === normalize(value));
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 mb-4">
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        {words.map((w, i) => (
          <span key={i} className="bg-muted text-muted-foreground px-3 py-1 rounded-md text-sm font-semibold">{w}</span>
        ))}
      </div>
      ...
      {validated && (
        <p className={`mt-2 text-sm font-semibold ${isCorrect ? "text-success" : "text-destructive"}`}>
          {isCorrect ? "✅ Bonne réponse !" : `❌ Réponse correcte : ${acceptedAnswers[0]}`}
        </p>
      )}
    </div>
  );
